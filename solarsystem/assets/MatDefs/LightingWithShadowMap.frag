...
varying vec4 ShadowSum;
varying float ShadowMapLightRamp;
...
void main(){
    ...
    // ***********************
      ...
      vec4 specularColor = vec4(1.0);
    ...
    #ifdef SHADOWMAP
    	vec4 shadowColor = texture2D(m_ShadowMap, newTexCoord);
	#else
       ...
       gl_FragColor =  AmbientSum * diffuseColor +
                       ...
                       //ShadowSum * ( shadowColor * (1 - light.x) );
                       ShadowSum * shadowColor * clamp(1.0 - (light.x / ShadowMapLightRamp), 0.0, 1.0);
                       
                       // This will make it glow all the time, this might be useful for something else.
                       // ShadowSum * shadowColor * sqrt(pow(10,2) - pow(light.x,2));
